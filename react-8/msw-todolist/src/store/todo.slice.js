...
const initialState = {
    ...
    //-->  addTodoModal 에서 따로 useSelector 로 가져와서 사용해주는 것을 화인할 수 있다  -->  이 addTodoState 로 여러가지 조건을 만들 수 있는 것이다
    addTodoState: {
    ...
    /*
        투두가 요청중이라면 : loading 을 true 로 바꿔야한다  -->  전역상태이기에 dispatch 해야한다  -->  loading  -->  dispatch
        투두가 성공했다면 : loading 은 false 로 바꾸고, done 은 true 로 바꿔줘야한다, todo 배열에는 값 채워줘야한다  -->  [...state, newTodo]  -->  dispatch
        투두가 실패했다면 : loading 은 false 로 바꾸고, done 은 true 로 바꿔줘야한다, error 에 에러메세지를 넣어야한다  -->  error.message  -->  dispatch
...
//-->  "todo/addTodo"  -->  로거에서 이름 기본값이 된다 (action 이 일어나는 것을 알려주는 이름이다  -->  logger 에서 확인 가능)
//-->  이 addTodo 친구의 상태 (로딩, 성공, 실패) 를 extraReducers 에 작성해줬다
