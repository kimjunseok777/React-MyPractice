...
/*
    ...
    post : 모두 전달할 수 있다  -->  body 안에 있는 내용물로도 되고, 주소로도 전달할 수 있다
...
//==>  post 로 "/api/user/login" 라고 백엔드 주소, 즉, 이름을 정해준 것이다  -->  프론트엔드가 여기에 데이터를 보내면 로그인이 되는 것이다 (fetch , axios ... )
...
export const login = http.post("/api/user/login", async ({ request }) => {
    ...
    //-->  로그인 데이터는 "주소" 로 전송하면 안된다 / "body" 데이터로 전송해야한다
