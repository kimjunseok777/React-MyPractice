...
const SignInForm = () => {
    ...
    const handlePressSignIn = async (data) => {
        ...
        try{ //==> 에러 핸들링 하기 위해 try , catch 사용해준 것이다
            ...
            // "/api/user/login"  -->  이 주소에 데이터를 요청하는 것이다
            const response = await fetch("/api/user/login", {
                //-->  fetch 는 자바스크리븥에서 내장된 "빌트인객체" 이다 (Math.random 같은 것처럼 아예 자바스크립트에서 내장된 기능)
                //-->  "/api/user/login" 는 백엔드 주소이다 (우리가 데이터를 보내는 목적지이다  -->  아래 있는 "post" 라는 메소드로 body 데이터를 보내는 것이다)
                /*
                    fetch :
                        -->  fetch 는 데이터를 응답받으면 json 형태로 오지 않고, Promise 형태의 "body" 라고 하는 응답데이터 형태로 온다
                    ...
                    body :
                        post 나 patch 등의 요청 등의 내부에 데이터를 포장해서  -->  즉, 데이터를 안에 넣어서 보내게 되는데, 이 요청이 (body) https 통신에서는 "암호화" 가 된다
