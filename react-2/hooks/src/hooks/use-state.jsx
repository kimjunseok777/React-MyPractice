...
const UseState = () => {
    ...
    //         console.log("거짓입니다")
    //     }
    // } //--> 상태를 사용하지 않고 버튼 출력값이 "진실입니다" 로 유지되는 것이 싫다면, 이렇게 함수 안에서 분기를 치는 방법이 있다
    ...
    const onPressChangeState = () => {
        ...
        //-->  "비동기" 로 되어있기 때문에, state 가 변경되고 리랜더링 되기 전까지 값이 바뀌지 않는다  -->  여전히 값이 true 인 것이다
        //-->  onPressChangeState 함수가 모두 종료되어야, 화면을 리랜더링 하면서 !state 인 false 로 변한다
    ...
    console.log(state)  //-->  이렇게 코드 블럭 바깥에서 콘솔을 찍으면 true 와 false 가 정상적으로 찍힌다
