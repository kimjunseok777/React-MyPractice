...
export const store = configureStore({
    ...
    */
    ...
    //==>  middleware 는 익명함수로 작성해준 것을 확인할 수 있다  -->  () => {}
    //==>  여기서 logger 는 "npm i redux-logger" 로 설치하고, import 받아온 것이다

    // rtk 는 이미 다양한 미들웨어가 설치된 상태이다  -->  따라서 기존에 있는 미들웨어에 로거라는 기능을 합치기 위해 concat 을 사용한다 (합쳐줘야한다)
    // getDefaultMiddleware  -->  rtk 의 기존 미들웨어를 가지고 오는 함수이다  -->  즉, 추가를 하려면 기존에 있는 미들웨어랑 합쳐야한다는 뜻이다  -->  concat 을 사용하면 된다
    //-->  기존에 있는 미들웨어를 매개변수로 가져오고, concat 함수로 logger 를 합쳐준 것이다  -->  이제 logger 기능이 실행하는 것을 확인할 수 있다
})