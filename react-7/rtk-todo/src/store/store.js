...
export const store = configureStore({
    ...
    /*
        ...
        env 는 "환경변수" 를 의미한다  -->  환경변수 : 코드의 동작에 영향을 미친는 변수, 동적으로 변경하는 값이다 (시스템 환경에 따라 값이 변경될 수 있다는 의미이다)
                                                          -->  일반변수는 선언할 때 "코드" 안에다가 선언하지만, 환경변수는 "컴퓨터" 에다가 선언하는 변수이다

        * 실제로 사용자들에게 배포하는 것은 bundle 된 코드이다  -->  "npm run build" 를 해서 나온 결과물을 배포한다
        ==>  이 결과물 때는 "NODE_ENV" 의 값이 "production" 이다  -->  즉 NODE_ENV = "production" 이라고 보면 된다 (사용자가 실제로 사용하는 환경은 production 이다)
    ...
    */
    ...
    // getDefaultMiddleware  -->  rtk 의 기존 미들웨어를 가지고 오는 함수이다  -->  즉, 추가를 하려면 기존에 있는 미들웨어랑 합쳐야한다는 뜻이다 (concat)
})